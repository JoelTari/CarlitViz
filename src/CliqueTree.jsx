...
function CliqueTree(){
  ...
  onMount(() =>{
    ...
    simulation.on("tick", () => {
      ...
      // update links
      d3selections.separators.select("line")
        .attr("x1", (d) => d.source.x)
        .attr("y1", (d) => d.source.y)
        .attr("x2", (d) => d.target.x)
        .attr("y2", (d) => d.target.y);
      d3selections.separators.select("text")
    ...
    // start simulation (it was stop at the declaration)
    simulation.restart();
  })
  ...
  createEffect(() => {
    ...
    forces.links= d3.forceLink(CopiedSlamData().links)
                    .distance(forces.links.distance())
                    .strength(forces.links.strength())
    simulation.nodes(CopiedSlamData().nodes) // force applies on nodes
              .force( "link"
                      , forces.links .id((d) => d.id)
                    ) 
    ...
    d3selections.separators = d3selections.separators
      .selectAll(".gseparator")
      .data(CopiedSlamData().links)
      .join((enter) => {
    ...
    console.log("Reactive CliqueTree: new data.")
  })
